...
seven-segment displays in your submarine are malfunctioning; they must have been damaged during the escape. 
...
b and g are turned on: the only digit that uses two segments is 1, so it must mean segments 
c and f are meant to be on. With just that information, you still can't tell which wire (b/g) 
...
using the current wire/segment connections. Because 7 is the only digit that uses three segments, 
...
in the above example, there are 26 instances of digits that use 
...
function guessNumber(chars){
    if(guessEasyNumbers(chars) == -1)
        return -1;
    else{
        return guessEasyNumbers(chars);
    }
}
