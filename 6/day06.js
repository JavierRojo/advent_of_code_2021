...
such large numbers - maybe exponentially quickly? You should model their growth rate to be sure.
...
of several hundred nearby lanternfish (your puzzle input). For example, 
...
class Fish {
    ...
    static mergeGenerations(){
        Fish.allFishes = Fish.allFishes.concat(Fish.newFishes);
        Fish.newFishes = [];
    }

    static getFishes(){
        let fishes = [];
        Fish.allFishes.forEach(f => {
            fishes.push(f.cycle);            
        });
        return fishes;
    }
    ...
    tick(){
        this.cycle--;
        if(this.cycle<0){
            Fish.newFishes.push(new Fish(8)); // Un hijito ^^
            this.cycle = 6;
        }
    }
  }
...
//puzzle6(readRawData(getTestData()),80);
