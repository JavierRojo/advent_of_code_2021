...
function puzzle3(inputData){
    let o2 = numberFromBits(calculateO2(inputData,0));
    let co2 = numberFromBits(calculateCO2(inputData,0));
...
function calculateO2(inputData, index){
    if(inputData.length==1) return inputData[0];
    let b = getNBit(inputData,index);
    let res = moreOnes(b,true); //or false for co2, I guess?

    let filteredData;
    if(res==true){
        filteredData=filterList(inputData,index,1)
    }else{
        filteredData=filterList(inputData,index,0)
    }
    return calculateO2(filteredData,index+1);
}

function calculateCO2(inputData, index){
    ...
    return calculateCO2(filteredData,index+1);
}

function filterList(originalArray, position, b){
    //go for array, and if bit in position == b, save it
    let r = [];
    originalArray.forEach(element => {
        if(parseInt(element[position]) == parseInt(b)){
            r.push(element);
        }     
    });   
    return r;
}
function getNBit(originalArray, position){
    let r = [];
    originalArray.forEach(element => {
            r.push(element[position]);
    
    });
    return r;
}
function moreOnes(bitArray,defaultValue=true){
    let sum = 0;
    ...
    if(sum == N/2) return defaultValue;
}
